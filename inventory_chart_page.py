...
def render_chart_page():
    ...
    if selected_month_num:
        ...
        # ✅ Correct last day of the selected month
        last_day = calendar.monthrange(selected_year, selected_month_num)[1]
        total_days = pd.Series(range(1, last_day + 1))
    ...
    if selected_month_num:
        # one tick per day
        fig_line.update_xaxes(tickmode="array", tickvals=chart_df["x_label"].unique(), tickangle=0)
    elif selected_year != "ALL":
        # monthly ticks
        fig_line.update_xaxes(dtick="M1", tickformat="%b", ticklabelmode="period")
    # if Year == "ALL", let Plotly auto-handle
